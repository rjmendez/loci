...
def test_no_registered_function_is_ever_reported_dead(head_build):
    """43 @mcp.tool(), 31 register()d, 13 _SKILL_MAP, 6 FastAPI routes,
    1 resource, 1 custom_route = 95 registered functions. None may be dead."""
    store = head_build.store
    registered = {e.dst for e in store.edges_of_kind("REGISTERS")}
    assert len(registered) == 95, f"registration surface changed: {len(registered)}"
