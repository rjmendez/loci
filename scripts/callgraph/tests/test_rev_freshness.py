...
def test_rev_head_is_immune_to_a_concurrently_mid_edited_working_tree(tmp_path, monkeypatch):
    ...
    assert len(sources) == 143  # 128 -> ... -> 142 -> 143: compact, deploy, A/B eval, prefilter, guardian, procedure-learning, conflict-verify, pre-answer-entailment, consolidation-audit, reflection-triage, wiring-obligation, local-deep-think, redteam-harness, swarm-escalate, bench-local-models
