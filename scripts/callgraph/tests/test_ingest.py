...
def test_load_corpus_worktree_parses_143_files_with_no_errors():
    """127 -> 128 -> 129 -> 130 -> 131 -> 132 -> 133 -> ... -> 142 -> 143:
    ...
    classification), then scripts/issue_proposer.py (guarded reflection-loop
    issue proposer)."""
    sources, origin = ingest.load_corpus(rev=None)
    assert origin == "working tree"
    assert len(sources) == 143
