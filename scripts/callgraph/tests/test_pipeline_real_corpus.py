...
def test_build_is_clean_and_fast(head_build):
    assert head_build.meta.file_count == 143  # 128 -> ... -> 140 -> 142 -> 143: compact, deploy, A/B eval, prefilter, guardian, procedure-learning, conflict-verify, pre-answer-entailment, consolidation-audit, reflection-triage, wiring-obligation, local-deep-think, redteam-harness, swarm-escalate, issue-proposer
...
def test_module_level_function_count_matches_census_within_tolerance(head_build):
    ...
    # proposer) adds its module-level helpers to the census.
