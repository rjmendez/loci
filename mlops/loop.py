...
def _embedding_tune_ran() -> bool:
    """True when a fine-tune newer than the trigger script exists.
    ...
    """
    script = MLOPS / "run_contrastive.sh"
    emitted = script.stat().st_mtime if script.exists() else 0.0
    return any(d.is_dir() and d.stat().st_mtime > emitted
