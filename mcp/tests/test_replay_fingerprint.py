...
def test_flybrain_audit_fingerprint_extracts_dataset_scope():
    payload = flybrain_audit_fingerprint(
        ...
        '{"upstream_type":"FBbt_00003686","exclude_dbs":["hb","fafb"]}',
        '{"rows":[{"dataset":"flywire783","db":"fw"},{"dataset":"male_cns_v1_0","db":"mc"}]}',
    )
    assert payload is not None
    assert payload["replay_fingerprint"]
    assert payload["replay_fingerprint_version"] == "v1"
    assert payload["replay_dataset_scope"]["included_symbols"] == ["fw", "mc"]
    assert payload["replay_dataset_scope"]["version_ids_seen"] == ["flywire783", "male_cns_v1_0"]
