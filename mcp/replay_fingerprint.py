...
def _canonicalize(value: Any, *, key_name: str = "") -> Any:
    if isinstance(value, dict):
        ...
        for key in sorted(value.keys(), key=lambda k: str(k)):
            k = str(key)
            out[k] = _canonicalize(value[key], key_name=k.lower())
        return out
    if isinstance(value, (list, tuple, set)):
        canonical_items = [_canonicalize(v, key_name=key_name) for v in value]
        if key_name in _SET_LIKE_KEYS:
            uniq = {json.dumps(v, sort_keys=True, separators=(",", ":"), ensure_ascii=True) for v in canonical_items}
            return [json.loads(item) for item in sorted(uniq)]
...
def _extract_dataset_scope(output_obj: Any) -> dict:
    ...
    walk(output_obj)
    return {
        "included_symbols": sorted(symbols),
        "version_ids_seen": sorted(versions),
    }
...
def apply_finding_fingerprints(
    ...
    *,
    ...
    Only writes fields for scoped records:
    - FlyBrain provenance metadata -> ``metadata.flybrain_provenance.replay_fingerprint``
    - Explicit provenance-bearing findings -> ``metadata.provenance_access_path_fingerprint``
    """
