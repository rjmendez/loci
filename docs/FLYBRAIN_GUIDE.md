...
| `mc` | `snapshots/mc/male-cns_v1.0` | planned (CC-BY-4.0) | `connectivity_tier`, `neurotransmitter_dominance`, `region_specialization_tier` | `flybrain-mc-training-samples/v1` |
| `mv` | `snapshots/mv/manc_v1.0` | planned (CC-BY-4.0) | `connectivity_tier`, `neurotransmitter_dominance`, `region_specialization_tier` | `flybrain-mv-training-samples/v1` |
| `ol` | `snapshots/ol/optic_lobe_v1.1` | planned (CC-BY-4.0) | `connectivity_tier`, `neurotransmitter_dominance` | `flybrain-ol-training-samples/v1` |
| `fafb` | `snapshots/fafb/catmaid_fafb` | deferred (never active) | none | none |

Planned datasets need `allow_planned=True` until their promotion gates (SC3 threshold reports, grouped-split trivial-baseline gate, AB AC1-AC5) pass. Larval (`l1em`) regions are never mapped to adult neuropils. `mc` and `ol` image the same male specimen but stay separate pinned versions; their rows are never merged without an explicit bodyId crosswalk. The `mc`, `mv` and `ol` builders keep the body id out of `input_text` (FlyEM body ids track neuron size, so they leak connectivity labels) and pick capped subsets in sha256(body id) order.
