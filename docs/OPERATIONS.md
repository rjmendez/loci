...
| `LOCI_OLLAMA_VERIFY_MODEL` | `qwen3.8:latest` | verify-tier model (`mcp/verify.py` via `mcp/backends.py:ollama_verify_model`). Hardening default: critical verify no longer silently inherits `LOCI_OLLAMA_GEN_MODEL`; set explicitly if you intentionally want a different (including qwen2.5) tier |
| `LOCI_OLLAMA_COMPRESS_MODEL` | `qwen3.8:latest` | compress-tier model (`mcp/text_ops.py` via `mcp/backends.py:ollama_compress_model`). Hardening default: critical compress no longer silently inherits `LOCI_OLLAMA_GEN_MODEL`; set explicitly to opt into another model |
...
| `LOCI_CLOUD_TIER_SUPERVISOR_MODEL` | `qwen3.8:latest` (via verify model default) | local supervisor model used to triage/dispatch unspecified-model calls by role (`triage`, `coding`, `reasoning`, `synthesis`, `redteam`) before cloud escalation |
...
- **Supervisor role**: a stronger local model (default `qwen3.8:latest`) triages
...
To prevent swarm abuse from untrusted callers, the MCP wrapper now enforces hard
resource caps before dispatch:

- `LOCI_SWARM_MAX_FANOUT` (default `64`, hard max `256`)
- `LOCI_SWARM_MAX_SEEDS` (default `8`, hard max `32`)
- `LOCI_SWARM_MAX_SELF_CONSISTENCY_SAMPLES` (default `5`, hard max `16`)
- `LOCI_SWARM_MAX_REDUCE_GROUP_SIZE` (default `32`, hard max `128`)
- `LOCI_SWARM_MAX_INFLIGHT` (default `2`, hard max `32`)

When inputs exceed caps, `swarm_reason` clamps them and annotates the response
with `orchestration_limits.applied`. When global in-flight capacity is full, it
returns a degraded JSON response immediately (`busy`) instead of queueing
unbounded work.
