...
- `loci_verdicts` — claim validation history. A 384-dim Cosine `dense` vector filled by a deterministic `hash_embed` (no model, no network). Name, dimension, vector name, embedder and creation all live in `mcp/memcheck/vectors.py`; every writer (the memcheck hook, `mcp/verdict_ops.py`, and `server.py`'s record path) calls its `ensure_collection`, so no writer can pin a dimension the others fail against. The vector is structural only — every read is an exact uuid5 point-id lookup or a payload scroll, so nothing searches it semantically. A collection existing at another dimension raises `VerdictDimensionMismatch` at construction, and `memory_health` reports it as a `fail`. Until the collection exists `memory_health` reports "not yet created", which is benign.
